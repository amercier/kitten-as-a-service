...
async function searchImages(accKey, q, pictureConfig, searchIndex, count) {
  const bing = bingApi({
...
function fetchAndStreamRandomJpegImage(accKey, q, pictureConfig, onData, onAbort) {
  ...
  return promiseRetry(async (retry) => {
    ...
    const candidates = await searchImages(accKey, q, pictureConfig, searchIndex, count)
      .catch((err) => {
...
function kittenPicture(apiKey, pictureConfig, response) {
  const q = 'kitten';
  let bar;
  ...
  fetchAndStreamRandomJpegImage(apiKey, q, pictureConfig, onData, () => response.end())
    .then(() => {
      log('   [ OK ] Done');
      response.end();
    })
...
module.exports = function makeApp({ apiKey }) {
  ...
  Object.keys(pictureConfigs).forEach(size => app.get(
    `/kitten-${size}.jpg`,
    (request, response) => kittenPicture(apiKey, pictureConfigs[size], response),
  ));
